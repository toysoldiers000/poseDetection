...
class PoseClassifier(object):
    ...
    def __init__(self,
                 ...
                 pose_embedder,
                 file_extension='csv',
                 file_separator=',',  # csv文件里分隔不同数据的符号
                 n_landmarks=33,
                 n_dimensions=3,
    ...
    def _load_pose_samples(self,
        ...
        for file_name in file_names:
            ...
            class_name = file_name[:-(len(file_extension) + 1)]  # classname是动作名称，每个csv文件是一个动作
    ...
    def __call__(self, pose_landmarks):
        ...
        Classification is done in two stages:
          * First we pick top-N samples by MAX distance. It allows to remove samples
            ...
            other direction.    移除和目标pose很像但是其他方向上的关节弯曲很少
          * Then we pick top-N samples by MEAN distance. After outliers are removed
            on a previous step, we can pick samples that are closes on average.

        Args:
          pose_landmarks: NumPy array with 3D landmarks of shape (N, 3).    输入一个动作坐标点，外面的循环会反复将每一个pose坐标点输入
        ...
        flipped_pose_embedding = self._pose_embedder(pose_landmarks * np.array([-1, 1, 1]))  # 镜像的动作embedding
        ...
        for sample_idx, sample in enumerate(self._pose_samples):
            max_dist = min(
                np.max(np.abs(sample.embedding - pose_embedding) * self._axes_weights),  # 最长的其中之一的关键之间的距离
...
class PoseClassificationVisualizer(object):
    ...
    def __init__(self,
                 ...
                 counter_font_path='https://gitee.com/mirrors_googlefonts/roboto/raw/main/src/hinted/Roboto-Regular.ttf?row=true',
        ...
        for pose_class_name in self._pose_class_names:
            ...
            with open(csv_out_path, 'w') as csv_out_file:
                ...
                for image_name in tqdm.tqdm(image_names):
                    ...
                    output_frame = np.concatenate((output_frame, projection_xz), axis=1)#对比图
